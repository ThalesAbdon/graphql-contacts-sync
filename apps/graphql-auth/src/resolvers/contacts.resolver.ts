...
export class ContactResolver {
    ...
    @UseGuards(AuthGuard)
    async sendContactsMongodb(@Args('input') input: Contacts): Promise<ContactsResponseOutput>{
        return await this.sendContactsUsecase.execute(input,process.env.ROUTE_MONGO)
    }

    @Mutation(() => ContactsResponseOutput) 
    @UseGuards(AuthGuard)
    async sendContactsMysql(@Args('input') input: Contacts): Promise<ContactsResponseOutput>{
        return await this.sendContactsUsecase.execute(input,process.env.ROUTE_MYSQL)
    }
}
